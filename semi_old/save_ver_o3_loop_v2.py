...
if __name__ == '__main__': 
    ...
    with Pool(processes=4) as pool:
        result = pool.map(f, range((q-1)*quater, (q)*quater)) #len(day_mjd_lst) 
#        result = pool.map(f, range(13791,13796))
...
# organize resulting arrays and save in nc file
    ...
    ds.to_netcdf(path+'ver_o3_{}{}_quater{}.nc'.format(year[0], str(month[0]).zfill(2), q))
