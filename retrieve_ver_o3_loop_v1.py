...
image_lst = (np.random.uniform(0,len(day_mjd_lst), 1)).astype(int)
...
def f(i):
    ...
    if alt_chop_cond.sum() < 2:
        print('wrong alt range ({}), idx {}'.format(i, orbit100_idx))
        pass
    elif l1.isel(mjd=i).notnull().sum() == 0:
        print('l1 is all nan ({}), idx {}'.format(i, orbit100_idx))
        pass
    elif error.isel(mjd=i).notnull().sum() == 0:
        print('error is all nan ({}), idx {}'.format(i, orbit100_idx))
        pass
    
    else:
        try:
            print(i, 'out of', len(day_mjd_lst), 'in idx ', orbit100_idx)
...
ver_hat = xr.DataArray(np.stack([result[i][1] for i in range(len(result))]), 
                         coords=(mjd, z*1e-3), 
                         dims=('mjd', 'z'), name='volume emission rate', attrs={'units': 'photons cm-3 s-1'})
o3_clima = xr.DataArray(np.stack([result[i][17] for i in range(len(result))]), 
                         coords=(mjd, clima.z), 
                         dims=('mjd', 'clima_z'), name='o3 CMAM', attrs={'units': 'cm-3'})
...
ds = xr.Dataset({'ver': ver_hat, 
                 ...
                 'o3_iteration':(['mjd'], 
                       ...
                       {'long name': 'number of iterations on Levenberg Marquardt'}),  
                 'o3_gamma':(['mjd'], 
                       np.stack([result[i][15] for i in range(len(result))]),
                       {'long name': 'gamma value at final iteration'}),  
                 'o3_status':(['mjd'], 
...
filename = 'ver_o3_{}xx_v1p0.nc'.format(orbit100[orbit100_idx])
